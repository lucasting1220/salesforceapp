  ...
  const handleConnectAll = () => {
    ...
    const connectNext = () => {
      ...
      setTimeout(() => {
        ...
        setConnecting(null);
        i++;
        if (i < remaining.length) setTimeout(connectNext, 300);
      }, 800);
    };
    connectNext();
  };
  ...
  const handleFinish = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        setTimeout(() => onComplete(email), 1800);
      }
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-card overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "connect" && (
          <motion.div key="connect" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
            ...
            <div className="px-5 pb-5 shrink-0 border-t border-border pt-3">
              <p className="text-[10px] text-muted-foreground text-center mb-2">
                {connected.size === 0 ? "Connect at least one tool to continue" : `${connected.size} of ${TOOLS.length} connected`}
              </p>
